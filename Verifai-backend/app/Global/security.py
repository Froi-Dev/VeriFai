...
def refresh_token_for_generation(session_id: str, generation: int) -> tuple[str, str]:
    """Derive a server-recoverable refresh token for an already-created session.

    Recoverability lets concurrent refresh requests receive the same rotated token
    without storing the bearer secret in the database.
    """
    material = f"verifai-refresh:v1:{session_id}:{generation}".encode()
    digest = hmac.new(
        settings.jwt_secret.get_secret_value().encode(), material, hashlib.sha384
    ).digest()
